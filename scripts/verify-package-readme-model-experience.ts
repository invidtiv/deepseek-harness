...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/ssh/ssh-environments': { kind: 'none', reason: 'The registry stores deployment connection configuration and opens no connection; its consumers own every model-visible operation.' },
  ...
  'packages/peer/peer': { kind: 'indirect', reason: 'The service interface delegates all model rendering to dsh-tool-peer.' },
  'packages/peer/peer-remote': { kind: 'indirect', reason: 'The transport delegates all model rendering to dsh-tool-peer.' },
  ...
  'packages/client/ui-file-explorer': { kind: 'none', reason: 'Browser-side UI plugin layer; renders project listings and opens already-local files; registers nothing model-facing.' },
  'packages/client/ui-file-viewer': { kind: 'none', reason: 'Browser-side UI plugin layer; renders files the browser already opened and registers nothing model-facing.' },
  ...
  'packages/bundle/telegram-bundle': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; each inserted row\'s package owns its model-facing behavior.' },
  'packages/telegram/telegram': { kind: 'indirect', reason: 'The transport adapter submits Telegram messages as ordinary user messages to the topic\'s agent; it registers no prompt, schema, or result of its own.' },
