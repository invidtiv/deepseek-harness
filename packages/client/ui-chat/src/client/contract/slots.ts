...
export interface ChatViewInjected {
   ...
   * plugin is composed in (fulfilling immediately); an absent viewer reveals
   * the file as a right-panel document tab, with `options.line` landing the
   * reveal; a directory (the "Show in folder" gesture) and a composition
   * with neither surface keep the host OS default application: fulfills when
