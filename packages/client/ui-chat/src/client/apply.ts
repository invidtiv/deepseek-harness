    ...
    const disposeView = ctx.slots.register({
      ...
      inject: (sessionId: SessionId): ChatViewInjected => {
        ...
        return {
          ...
          // Files stay in the product, beside the conversation that produced
          // them. A file path opens in the in-app file viewer when that
          // plugin is composed in; an absent viewer reveals the file as a
          // right-panel document tab: a relative path, or an absolute one
          // inside the session's workspace, is addressed under this session's
          // scope, `dsh-resource://file/session/<id>/<path>`; an absolute path
          // elsewhere keeps its absolute spelling in the same Session's
          // address. Which tab type claims the address is the Sidebar's
          // decision, not this call site's. A line travels as a navigation
          // parameter, not as part of the address: the file is one piece of
          // content whether it is opened at its top or at line 400, so the
          // same tab is revealed and told where to land. A directory (the
          // "Show in folder" gesture) and a composition with neither surface
          // keep the Host's native opener.
          openFile: async (path, options) => {
            ...
            if (sidebarRight !== undefined) {
              ...
              if (options?.line === undefined) sidebarRight.openResource(url)
              else sidebarRight.openResource(url, { params: { line: options.line } })
              return
            }
            const result = await ctx.remote.session.openWorkspacePath({ path: resolved })
            if (!result.ok) throw new Error(`path open failed: ${result.error.message}`)
