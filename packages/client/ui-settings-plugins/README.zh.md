...
**SSH 环境**卡片以行的形式编辑部署的具名 SSH 环境——每行一个标识与一个 OpenSSH 目标——支持添加与移除。该命名空间只包含一张映射，因此卡片暂存整组行并在一次保存中写入；卡片不编辑的连接选项（`identityFile`、`proxyJump`、host-key 策略、超时）保留在已存条目中并原样写回。
