...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and declares the six root-scoped frame slots', async () => {
    ...
    expect(slots.spec('explorer')).toEqual({ kind: 'single', scope: 'root' })
    expect(slots.spec('fileViewer')).toEqual({ kind: 'single', scope: 'root' })
  ...
  it('forwards the frame panels through the one shared action set', async () => {
    const { ctx, slots, rendererHost } = await bench()
    const fiber = ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    ...
    expect(instance.getSnapshot().layoutInfo.explorer).toBe(300)
    layout.openFileViewer()
    expect(instance.getSnapshot().layoutInfo.fileViewer).toBe(720)
    layout.closeFileViewer()
    expect(instance.getSnapshot().layoutInfo.fileViewer).toBe(0)
    layout.openRightbar(true, false)
    expect(instance.getSnapshot().layoutInfo).toMatchObject({ rightbarShown: true, rightbarTrack: true })
    layout.closeRightbar()
    expect(instance.getSnapshot().layoutInfo.rightbarShown).toBe(false)
  })
  ...
  it('teardown unwinds the service, the root registration, and the child declarations', async () => {
    ...
    expect(slots.spec('explorer')).toBeUndefined()
    expect(slots.spec('fileViewer')).toBeUndefined()
