...
describe('createLayoutStore', () => {
  it('initializes the sidebar at its default width, right panels closed, wide viewport assumed', () => {
    ...
    expect(store.getSnapshot()).toEqual({
      sidebar: SIDEBAR_DEFAULT, details: 0, fileViewer: 0, explorer: 0, narrow: false, narrowExpanded: false,
    })
  })
  ...
  it('setSidebar/setDetails/setFileViewer/setExplorer clamp into the contract ranges', () => {
    ...
    expect(store.getSnapshot().fileViewer).toBe(FILE_VIEWER_MIN)
    actions.setFileViewer(9999)
    expect(store.getSnapshot().fileViewer).toBe(FILE_VIEWER_MAX)
    actions.setExplorer(1)
    expect(store.getSnapshot().explorer).toBe(EXPLORER_MIN)
    actions.setExplorer(9999)
    expect(store.getSnapshot().explorer).toBe(EXPLORER_MAX)
  })
  ...
  it('narrow toggleSidebar flips only the re-expand override; the width preference survives', () => {
    ...
    actions.toggleSidebar()
    expect(store.getSnapshot()).toEqual({ sidebar: 400, details: 0, fileViewer: 0, explorer: 0, narrow: true, narrowExpanded: true })
    actions.toggleSidebar()
  ...
  it('openFileViewer uses the contract default, preserves an open width, and closeFileViewer zeroes', () => {
    const { store, actions } = createLayoutStore().create()
    actions.openFileViewer()
    expect(store.getSnapshot().fileViewer).toBe(FILE_VIEWER_DEFAULT)
    actions.setFileViewer(800)
    actions.openFileViewer()
    expect(store.getSnapshot().fileViewer).toBe(800)
    actions.closeFileViewer()
    expect(store.getSnapshot().fileViewer).toBe(0)
  })

  it('toggleExplorer flips closed <-> contract default (drag width forgotten)', () => {
    const { store, actions } = createLayoutStore().create()
    actions.setExplorer(480)
    actions.toggleExplorer()
    expect(store.getSnapshot().explorer).toBe(0)
    actions.toggleExplorer()
    expect(store.getSnapshot().explorer).toBe(EXPLORER_DEFAULT)
  })
