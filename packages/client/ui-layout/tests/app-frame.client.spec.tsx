...
import type { ExplorerOwnerProps, MainPanelId, RightbarOwnerProps, SidebarOwnerProps } from '../src/client/index.ts'
...
function mountFrame(windowWidth = frameWidth) {
  ...
  return {
    ...
    explorerOwner: () => slotCalls.findLast(c => c.key === 'explorer')!.props as ExplorerOwnerProps,
  }
}

/** Resolved grid tracks in column order (the center track is fluid and not reported). */
function tracks(frame: HTMLElement): { sidebar: number; explorer: number; fileViewer: number; rightbar: number } {
  const match = /^([\d.]+)px minmax\(0, 1fr\) ([\d.]+)px ([\d.]+)px ([\d.]+)px$/.exec(frame.style.gridTemplateColumns)
  if (match === null) throw new Error(`unexpected template: ${frame.style.gridTemplateColumns}`)
  return {
    sidebar: Number(match[1]),
    explorer: Number(match[2]),
    fileViewer: Number(match[3]),
    rightbar: Number(match[4]),
  }
}

function handleFor(frame: HTMLElement, side: 'sidebar' | 'explorer' | 'fileViewer' | 'rightbar'): HTMLElement {
...
describe('AppFrame', () => {
  ...
  it('renders owner props for the default sidebar, the explorer rail, and the prospective right panel', () => {
    const { frame, rightOwner, sidebarOwner, explorerOwner, slotCalls } = mountFrame()
    expect(tracks(frame)).toEqual({ sidebar: 280, explorer: 44, fileViewer: 0, rightbar: 0 })
    expect(sidebarOwner()).toEqual({ collapsed: false, width: 280 })
    expect(explorerOwner()).toEqual({ collapsed: true, width: 44 })
    ...
    expect(slotCalls.find(c => c.key === 'fileViewer')).toEqual({ key: 'fileViewer', props: {}, options: undefined })
  })

  it('renders every column outlet without a current Session', () => {
    selectedSession = undefined
    ...
    expect(getByTestId('fileViewer-content')).toBeTruthy()
    expect(getByTestId('rightbar-content')).toBeTruthy()
    expect(tracks(frame)).toEqual({ sidebar: 280, explorer: 44, fileViewer: 0, rightbar: 0 })
  })

  it('renders every occupant before workspace baselines settle', () => {
  ...
  it('keeps Windows caption controls mounted with a zero-width collapsed column', () => {
    ...
    try {
      ...
      expect(tracks(frame).sidebar).toBe(0)
  ...
  it('keeps the closed sidebar mounted at its 56px rail without a handle', () => {
    const { frame, instance, sidebarOwner, getByTestId } = mountFrame()
    act(() => { instance.actions.toggleSidebar() })
    expect(tracks(frame)).toEqual({ sidebar: 56, explorer: 44, fileViewer: 0, rightbar: 0 })
...
describe('AppFrame explorer and file viewer columns', () => {
  it('expands the explorer from its rail and reports open owner props', () => {
    const { frame, instance, explorerOwner } = mountFrame()
    expect(explorerOwner()).toEqual({ collapsed: true, width: 44 })
    act(() => { instance.actions.toggleExplorer() })
    expect(tracks(frame)).toEqual({ sidebar: 280, explorer: 300, fileViewer: 0, rightbar: 0 })
    expect(explorerOwner()).toEqual({ collapsed: false, width: 300 })
    expect(handleFor(frame, 'explorer')).toBeTruthy()
    act(() => { instance.actions.toggleExplorer() })
    expect(tracks(frame)).toEqual({ sidebar: 280, explorer: 44, fileViewer: 0, rightbar: 0 })
    expect(frame.querySelector('[data-side="explorer"]')).toBeNull()
  })

  it('widens the explorer rightward through pointer input', () => {
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.toggleExplorer() })
    const handle = handleFor(frame, 'explorer')
    expect(handle.style.left).toBe(`${1920 - 300}px`)
    drag(handle, 1620, 1700)
    expect(instance.getSnapshot().layoutInfo.explorer).toBe(380)
    expect(tracks(frame).explorer).toBe(380)
  })

  it('opens the file viewer at its default width and keeps it mounted at zero', () => {
    const { frame, instance, getByTestId } = mountFrame()
    act(() => { instance.actions.openFileViewer() })
    expect(tracks(frame)).toEqual({ sidebar: 280, explorer: 44, fileViewer: 720, rightbar: 0 })
    expect(getByTestId('fileViewer-content')).toBeTruthy()
    expect(handleFor(frame, 'fileViewer')).toBeTruthy()
    act(() => { instance.actions.closeFileViewer() })
    expect(tracks(frame)).toEqual({ sidebar: 280, explorer: 44, fileViewer: 0, rightbar: 0 })
    expect(getByTestId('fileViewer-content')).toBeTruthy()
    expect(frame.querySelector('[data-side="fileViewer"]')).toBeNull()
  })

  it('concedes the explorer only after the right columns are gone', () => {
    // 280 + 500 + 640 leaves the center 500: the explorer shrinks, then the file
    // viewer and rightbar track have already been given up.
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.setExplorer(500); instance.actions.openFileViewer(); instance.actions.openRightbar(true, false) })
    resize(1500)
    expect(tracks(frame)).toEqual({ sidebar: 280, explorer: 500, fileViewer: 0, rightbar: 0 })
    resize(1180)
    expect(tracks(frame).explorer).toBe(260)
    resize(900)
    expect(tracks(frame).explorer).toBe(44)
    resize(1920)
    expect(tracks(frame).explorer).toBe(500)
  })
})
...
describe('AppFrame normal width concessions', () => {
  it('measures the frame, not the window, before choosing the first-open preference', () => {
    ...
    expect(rightOwner()).toEqual({ width: 0, viewportWidth: 1000, canShow: false })
  ...
  it('shrinks the right panel within its remaining room before it concedes its track', () => {
    const { frame, instance, rightOwner } = mountFrame()
    act(() => { instance.actions.openRightbar(true, false) })
    resize(1440)
    expect(tracks(frame)).toEqual({ sidebar: 280, explorer: 44, fileViewer: 0, rightbar: 476 })
    expect(rightOwner()).toEqual({ width: 476, viewportWidth: 1440, canShow: true })
    resize(1264)
    expect(tracks(frame).rightbar).toBe(300)
    resize(1263)
    expect(tracks(frame).rightbar).toBe(0)
    expect(rightOwner()).toEqual({ width: 0, viewportWidth: 1263, canShow: false })
    ...
    expect(tracks(frame)).toEqual({ sidebar: 56, explorer: 44, fileViewer: 0, rightbar: 0 })
    resize(1920)
    expect(tracks(frame).sidebar).toBe(280)
  })

  it.each([[1264, 300, true], [1263, 0, false]] as const)('reports right panel eligibility at %ipx', (width, rightbar, canShow) => {
    frameWidth = width
    const { rightOwner } = mountFrame()
    expect(rightOwner()).toEqual({ width: rightbar, viewportWidth: width, canShow })
  })

  it('keeps the right panel trackless on a narrow frame while still reporting it shown', () => {
    frameWidth = 800
    const { frame, instance, rightOwner } = mountFrame()
    expect(rightOwner()).toEqual({ width: 0, viewportWidth: 800, canShow: false })
    act(() => { instance.actions.openRightbar(true, false) })
    expect(tracks(frame)).toEqual({ sidebar: 56, explorer: 44, fileViewer: 0, rightbar: 0 })
    expect(instance.getSnapshot().layoutInfo).toMatchObject({ rightbarShown: true, narrowExpanded: false, rightbar: 360 })
  ...
  it('auto-collapses only below 1024px and preserves the wide sidebar preference', () => {
    ...
    resize(1024)
    expect(tracks(frame).sidebar).toBe(400)
    resize(1023)
    expect(tracks(frame).sidebar).toBe(56)
    act(() => { instance.actions.toggleSidebar() })
    expect(tracks(frame).sidebar).toBe(400)
    resize(980)
    expect(tracks(frame).sidebar).toBe(400)
...
describe('AppFrame right panel presentation', () => {
  it('releases the fullscreen track with the instant marker while clearing fullscreen', () => {
    ...
    expect(tracks(frame)).toEqual({ sidebar: 280, explorer: 44, fileViewer: 0, rightbar: 864 })
    act(() => { instance.actions.closeRightbar() })
    expect(tracks(frame).rightbar).toBe(0)
  ...
  it('marks restoration instant without suppressing the following normal close', () => {
    ...
    act(() => { instance.actions.openRightbar(true, true) })
    act(() => { instance.actions.openRightbar(true, false) })
    expect(tracks(frame).rightbar).toBe(864)
    expect(frame.dataset.rightbarFullscreen).toBeUndefined()
    expect(frame.dataset.rightbarInstant).toBe('true')
    act(() => { instance.actions.closeRightbar() })
    expect(tracks(frame).rightbar).toBe(0)
  ...
  it('keeps resolved panel width independent of the requested track', () => {
    ...
    resize(1400)
    expect(tracks(frame).rightbar).toBe(0)
    expect(rightOwner().width).toBe(436)
    const handle = handleFor(frame, 'rightbar')
    expect(handle.style.left).toBe('964px')
    drag(handle, 964, 974)
    expect(instance.getSnapshot().layoutInfo.rightbar).toBe(426)
    expect(rightOwner().width).toBe(426)
    expect(tracks(frame).rightbar).toBe(0)
  })
})
...
describe('AppFrame pointer resizing', () => {
  it('updates columns during the gesture and freezes the drag-start width', () => {
    ...
    act(flushFrames)
    expect(tracks(frame).sidebar).toBe(340)
    pointer(handle, 'pointermove', 360)
    act(flushFrames)
    expect(tracks(frame).sidebar).toBe(360)
  ...
  it('starts a conceded right drag at its actual width, shared by panel and track', () => {
    ...
    resize(1440)
    const handle = handleFor(frame, 'rightbar')
    expect(rightOwner().width).toBe(476)
    expect(tracks(frame).rightbar).toBe(476)
    expect(handle.style.left).toBe('964px')
    drag(handle, 964, 974)
    expect(instance.getSnapshot().layoutInfo.rightbar).toBe(466)
    expect(rightOwner().width).toBe(466)
    expect(tracks(frame).rightbar).toBe(466)
    expect(handle.style.left).toBe('974px')
  })
  ...
  it('widens to the 70% limit and shrinks to 300px through pointer input', () => {
    ...
    expect(tracks(frame).rightbar).toBe(2100)
