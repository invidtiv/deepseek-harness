...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  ...
  interface SlotMap {
     ...
     */
    'explorer': { kind: 'single'; scope: 'root'; owner: ExplorerOwnerProps }
    /**
     * The file-viewer column between the explorer and the right panel, shown
     * when the layout opens it. OCCUPIED by ui-file-viewer's FileViewer, which
     * shows the active file's contents. Absent an occupant the column renders
     * nothing. Global scope: the drawer is a single frame-wide surface (not
     * per-session), so it receives the global `useSessions`/`useWorkspaces`
     * kit but no `sessionId`.
     *
     * No owner props: the occupant reads its own store and `ctx.layout` owns
     * whether the column is open.
     */
    'fileViewer': { kind: 'single'; scope: 'root'; owner: FileViewerOwnerProps }
    /**
...
/** File-viewer owner share: empty — the occupant reads its own store. */
...
export interface RightbarOwnerProps {
  ...
  /**
   * Whether a normal right panel can retain RIGHTBAR_MIN px beside a CENTER_MIN
   * center. Before a narrow opening, includes the space from collapsing the
   * left sidebar.
   */
  canShow: boolean
}
  ...
  ctx.effect(() => {
    ...
    const disposeRegistration = ctx.slots.register({
      ...
      children: {
        ...
        'fileViewer': { kind: 'single', scope: 'root' },
