...
export function createLayoutStore(): EngineStoreHandle<LayoutState, LayoutActions>  {
  const handle = defineStore({
    ...
    actions: {
      ...
      setFileViewer: (d, px: number) => { d.fileViewer = clampWidth(px, FILE_VIEWER_MIN, FILE_VIEWER_MAX) },
      ...
      openFileViewer: (d) => { if (d.fileViewer === 0) d.fileViewer = FILE_VIEWER_DEFAULT },
      closeFileViewer: (d) => { d.fileViewer = 0 },
    },
  })
  return handle
}
