...
拦截是调用方的选择，而非本包的行为：[ui-conversation](../ui-conversation/README.zh.md) 会先把点击路径按会话 cwd 解析，当 `ctx.get('fileViewer')` 可用时路由到 `open`，否则回退到宿主打开器（`openPath`）。因此将本包从组合中移除后，每个文件链接都会恢复为“用系统默认应用打开”的旧行为，调用方无需任何改动。目录场景（“在文件夹中显示”）不是文件读取，始终保留宿主打开器。

`readFile` 与 `openPath` 一样受 loopback 限制：它会读取任意宿主文件内容，因此非 loopback 的浏览器客户端会收到 `403`，抽屉显示其错误状态。契约见 [slot 系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.zh.md)。
