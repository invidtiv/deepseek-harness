...
/** Compact byte count for the metadata row (unit abbreviations are locale-neutral). */
    ...
    <div className={css.root} data-status={activeFile.status} data-render-mode={markdown ? renderMode : undefined}>
      ...
      {activeFile.status === 'ready' && (
        <div className={css.meta}>
          <span className={css.metaItem}>{activeFile.lang ?? 'text'}</span>
          <span className={css.metaItem}>{lineCount} {t('viewer.lines')}</span>
          <span className={css.metaItem}>{formatSize(activeFile.size)}</span>
