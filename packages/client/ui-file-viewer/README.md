...
Interception is the caller's choice, not this package's: [ui-conversation](../ui-conversation/README.md) resolves a click's path against the session cwd and, when `ctx.get('fileViewer')` resolves, routes it to `open`; otherwise it falls back to the Host opener (`openPath`). Composing this package out therefore restores the previous open-with-default-application behavior for every file link with zero caller edits. The directory case (`Show in folder`) is not a file read and keeps the Host opener regardless.

`readFile` is loopback-gated like `openPath`: it reads arbitrary host file contents, so a non-loopback browser client receives `403` and the drawer shows its error state. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
