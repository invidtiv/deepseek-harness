...
`resolve()` 在远端主机上规范化路径。`processPath()` 与 `fileUrl()` 在同一个远端命名空间中标识文件，并不授予主机侧访问能力。文件 URL 对字面的百分号、反斜杠和换行进行编码，保留原文件名。`processPathFromHostPath()` 返回 `undefined`，因此需要已安装可执行文件或引导程序的消费方必须显式提供远端产物。该提供方报告 `addressesHostFilesystem: false`，因此只能提供 OS 原生路径选择器的界面不会服务它。

当部署组合了 SSH 世界池时，每个操作都会路由到拥有其目标世界的连接，已打开的文本流也留在开启它的连接上。读取保留共享文件系统错误码。写入和编辑将已解析的逐次调用策略发送给辅助程序，由其规范化工作区并在原子修改所在位置执行策略；创建目录在提供逐次调用策略时转发该策略，否则按部署策略执行。传输丢失报告 I/O 失败；修改可能已经提交，不会自动重试。
