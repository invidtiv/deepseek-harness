...
一个 `ssh` 行服务一个世界。需要访问多台服务器的部署会把池行（`@deepseek-ai/dsh-ssh/worlds`）与每个环境各一个 `ssh` 行一同组合，每行各自携带 `environment` 或 `host`；池被组合时各行都会把自己的连接注册进池，未被 Loader 隔离的那一行充当该部署的默认连接。路由以目标为依据：工作区注册表的定位符——环境加远程目录——决定某个路径属于哪个世界，拥有该路径的最长定位符胜出，没有定位符认领的目标归默认连接，两个世界声明同一目录则被拒绝。目标所属世界没有已组合连接时会明确失败。
