...
import type {
  ApiProxy, ConfigurableProviderView, CredentialView, FileContents, FileListing, FileListingEntry,
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
   ...
   * binary-refusing: a directory, a missing path, and a read failure all
   * become their own wire error, while a file too large to show whole returns
   * a truncated prefix and a non-text file returns `binary` with empty
   ...
   * @returns the file contents, or a wire error.
   */
  async function readFile(
    ...
    try {
      ...
      if (signal.aborted) {
        return err(request, { code: 'cancelled', message: 'file read was aborted', details: { path } })
      }
      return err(request, {
        code: 'file-unreadable',
   ...
   * symlink targets probe per row, and the wire answer keeps the name-sorted
   * head plus the `truncated` flag. An unreadable or missing level — including
   * a non-directory target — maps onto the shared listing error code.
   * @param request - the listing request.
   * @param signal - caller/connection lifetime.
   * @returns the mixed listing, or a wire error.
   */
  async function listFiles(
    ...
    // session starts from, matching what host.describe reports.
    ...
    try {
      ...
      return ok(request, { path: target, entries, truncated: evicted })
    } catch (error) {
      if (signal.aborted) {
  ...
  return {
    ...
    host: {
      ...
      async readFile(request, signal) {
        return readFile(request, signal)
      },

      async listFiles(request, signal) {
        return listFiles(request, signal)
      },
    },
