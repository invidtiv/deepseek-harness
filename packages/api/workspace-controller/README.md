...
The `readFile` and `listFiles` verbs are served by `WorkspaceFileBrowse` over the composed filesystem, so the Web file explorer and viewer read the same execution world the agent tools do: reads are capped at 2 MiB with a truncation flag and refuse binary content through a NUL-byte check, listings keep the name-sorted head under a 1000-entry bound, and an absent listing path resolves to the configured default project root in the composed execution world, falling back to that world's root when it cannot read host files. Expected failures carry stable codes (`file-not-found`, `file-unreadable`, `directory-unreadable`, `cancelled`). The `environments` verb lists the deployment's named SSH environments for the workspace picker — the identity a remote workspace records and a remote session selects — or an empty list when no `ssh-environments` registry is composed.
